...
import { Candidate, CandidateList, ElectionData, templateTypes } from "./types"
...
const parseCandidateList = (rows: Row[]): { list: CandidateList; unconsumedRows: Row[] } | null => {
  ...
  while (rows.length > 0 && rows[0].getCell(3).text != "Listenname") {
    ...
    const positionMatch = (
      list.order == "alphabetical" ? rows[0].getCell(2) : rows[0].getCell(1)
    ).text.match(courseRegEx)
    if (!nameMatch || !positionMatch)
      throw new Error(`Kandidat nicht korrekt formatiert in Zeile: ${rows[0].values}`)
...
export const generateResultHtml = (electionData: ElectionData) => {
  const getSubsetHtml = (list: CandidateList, start: number, numberOfSeats?: number) => {
    const subsetCandidates = list.candidates
      .sort((a, b) => b.votes - a.votes)
      .slice(start, numberOfSeats ? start + numberOfSeats : undefined)
    return `<table>
        ${subsetCandidates
          .map(
            (candidate: Candidate) => `
  ...
  const getListHtml = (list: CandidateList) => `
    <h4>${list.name}</h4>
    <h5>${
      list.order == "alphabetical" ? "in alphabetischer Reihenfolge" : "in Listen-Reihenfolge"
    }</h5>
    <h6>Gewählt</h6>
