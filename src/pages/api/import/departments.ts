...
import importDepartments from "../basis/mutations/importDepartments"

/**
 * Import departments.
 * Request must contain department excel file.
...
const handler = api(async (req, res, ctx) => {
  try {
    ...
    try {
      res.status(200).json(await importDepartments(upload.id, ctx))
    } catch (error) {
      throw {
        statusCode: 400,
        ...error,
