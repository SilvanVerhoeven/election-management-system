...
import importFaculties from "../basis/mutations/importFaculties"

/**
 * Import faculties.
 * Request must contain faculty CSV file.
...
const handler = api(async (req, res, ctx) => {
  try {
    ...
    try {
      res.status(200).json(await importFaculties(upload.id, ctx))
    } catch (error) {
      throw {
        statusCode: 400,
        ...error,
